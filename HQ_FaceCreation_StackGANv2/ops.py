...
def dnet_basic(x, d_dim):
    ...
    y = Conv2D(d_dim, 3, strides=2, padding='same', kernel_initializer=truncateNormal)(x)
    y = LeakyReLU(alpha=0.2)(y)
    y = Conv2D(d_dim*2, 3, strides=2, padding='same', kernel_initializer=truncateNormal)(y)
    y = LeakyReLU(alpha=0.2)(y)
    y = Conv2D(d_dim*4, 3, strides=2, padding='same', kernel_initializer=truncateNormal)(y)
    y = LeakyReLU(alpha=0.2)(y)
    y = Conv2D(d_dim*8, 3, strides=2, padding='same', kernel_initializer=truncateNormal)(y)
    y = LeakyReLU(alpha=0.2)(y)
    return y
...
def gnet_upsample(y, g_dim):
    y = Conv2DTranspose(g_dim, 4, strides=2, kernel_initializer=truncateNormal, padding='same')(y)
    y = BatchNormalization(momentum=0.9, epsilon=1e-5)(y)
    y = Lambda(glu)(y) #128, 128, 32
    return y

def residual_block(x, dim):
    y = Conv2D(dim*2, 4, strides=1, padding='same', kernel_initializer=truncateNormal)(x)
    y = BatchNormalization(momentum=0.9, epsilon=1e-5)(y)
    y = Lambda(glu)(y)
    y = Conv2D(dim, 4, strides=1, padding='same', kernel_initializer=truncateNormal)(y)
    y = BatchNormalization(momentum=0.9, epsilon=1e-5)(y)
    return Add()([x,y])
